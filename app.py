...
# %%
with open(f"data/cluster_stats.pkl", "rb") as fp:
    cluster_stats = pickle.load(fp)

with open(f'data/diametros.pkl', 'rb') as f:
    diametros = pickle.load(f)

with open(f"data/municipios_per_cluster.pkl", "rb") as fp:
    municipios_per_cluster = pickle.load(fp)

with open("data/muni_info.pkl", "rb") as fp:
    muni_info = pickle.load(fp)

# %%
with open(f"data/cluster_800_alias.json", "r") as f:
...
# paleta_colores = ["#dac6fb", "#9beaf5","#cdf06a", "#f5ef32", "#f2af97", "#c6cdd1"]
# paleta_colores2 = ["#9259e5", "#5bc0db","#a7d650", "#facb0e", "#eb612c", "#3c3f40"]
        ...
        dbc.CardBody([
            ...
            dbc.Col(dcc.Graph(id='boxplot-graph')),
                dbc.Col([
                    ...
                    # html.Details([
                    # html.Summary('Seleccionar Municipios'),
                    # html.Br(),
                    # dbc.Col([
                    #     dbc.Checklist(
                    #         id='nombre-select',
                    #         options=dropdown_municipio_options,
                    #         labelStyle = {'display': 'block'}
                    #         )  
                    #     ])
                    # ])
...
def update_scatter(x_var, y_var, color_var, regions):
    ...
    if color_var == 'None':
        ...
        is_categorical = (df[color_var].dtype.name == 'object' or 
                          df[color_var].dtype.name == 'category' or 
                          df[color_var].nunique() < 10)
        
        if is_categorical:
            # Discrete color mapping
            unique_cats = df[color_var].unique()
            # Use a Plotly qualitative colorscale
    ...
    fig.update_layout(
        ...
        title = None,
        margin=dict(t=10),
        xaxis_title="Betweenness - RCD rutas",#variable_labels[x_var],
        yaxis_title="Betweenness - RCC rutas",#variable_labels[y_var],
        height=500,
        template='plotly_white',
        legend_font_size=20,
        hovermode='closest'
...
def update_boxplot(var, orderby, regions):
    ...
    # df["colors"] = [colores[x] for x in df.region_nombre]
    ...
    # fig = px.box(df, y="muni_nombre", x=var,
    #              hover_name=index_vals,
    #              color="region_nombre",
    ...
    # for xd, yd in zip(df[var], df["region_nombre"]):
    for muni, data in df.groupby("muni_nombre"):
        ...
        fig.add_trace(go.Box(
            ...
            line_width=1,
            showlegend=False,
            # showlegend=(region not in added_regions),  # Show legend only once per region
            # legendgroup=region,  # Group traces by region
            # legendgrouptitle=region,
            # legendgrouptitle_text=region if region not in added_regions else "",
            name=muni
            )
        )
    ...
    fig.update_layout(
        ...
        template='plotly_white',
        legend_font_size=20,
        xaxis_title="Máxima Distancia Mínima Pesada por Rutas",#variable_labels[var],
        yaxis_title= f"Departamento (ordenados por {order_label[orderby]})"#variable_labels["muni_nombre"],
    )
    fig.update_xaxes(title_font=dict(size=25), tickfont=dict(size=20))
    fig.update_yaxes(title_font=dict(size=25), tickfont=dict(size=20))
    return fig
